...
**`gh` account.** This repo must authenticate as `robwilde`; `rob-ee-wilde` is authenticated too and has no push access here, so a stray `gh auth switch` in any other project causes `remote: Permission to robwilde/can-eye-budget-v2.git denied to rob-ee-wilde`. Run `scripts/install-git-hooks.sh` once per clone: it pins a repo-local credential helper to `robwilde` via `gh auth token --user` (no global state touched, `rob-ee-wilde` keeps working elsewhere) and installs a `pre-push` guard that fails fast with the remedy. `gh` subcommands (`gh pr create`, `gh api`) read the *globally active* account and cannot be redirected by git config — run `scripts/gh-account-guard.sh --fix` before those if the guard warns. Bypass with `GH_ACCOUNT_GUARD=0 git push`. Never repoint `core.hooksPath`: GrumPHP owns `.git/hooks`.
