...
- **Transaction enrichment (#465):** brands live in the per-user `merchant_brands` sidecar keyed by `(user_id, merchant_key)` (`App\Models\MerchantBrand`, status `resolved`|`unresolved`|`vetoed`; `partial` is a flag on a resolved row). **Never write `transactions.merchant_name`**: it feeds `merchant_key`, which drives rules, the rule backlog and recurring clusters. `App\Services\MerchantBrands\DescriptorGate` decides what may leave the app and **sends only the redacted `MerchantSignature` form** (raw descriptors carry card masks, auth codes, policy/member numbers); it denies credits, transfer pairs, folded fees, transfer/BPAY/PayID/Osko/ATM/fee/income wording and bare payment-rail keys. `EnrichMerchantBrandsJob` (dispatched after CSV/Basiq/Redbark imports) queues `ResolveMerchantBrandJob` per key with ≥2 debit rows and no vetoed/fresh row, most frequent first, sized to the remaining budget; both run on the `enrichment` Horizon queue (listed in `config/horizon.php`). Retry windows: resolved 180d, partial 30d, unresolved 14d; vetoed never. Spend controls: `CONTEXT_DEV_ENRICHMENT_ENABLED` (default **false**) and `CONTEXT_DEV_DAILY_CREDIT_CAP` (default 500 credits per Australia/Brisbane day, `ContextDevCreditBudget`). UI: `TransactionList` shows logo + brand name beside the original descriptor, "Identify merchant" (user-requested lookup, still gated) and "Wrong merchant" (veto).
