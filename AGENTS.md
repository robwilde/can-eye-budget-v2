...
- **Transport gotcha:** the SDK's auto-discovered Guzzle client has `http_errors=true`, so every 4xx/5xx surfaces as `APIConnectionException` (no 429/5xx retry, no 404 → `NotFoundException`). `withApiKey()` injects Guzzle with `http_errors=false`; keep it. Retries are the SDK's: 408/409/429/5xx, max 2; 4xx validation errors are never retried. At the defaults the retries fire with effectively no backoff; only a numeric `Retry-After` adds a wait, and the SDK sleeps for it uncapped (the HTTP-date form is ignored) — mind that once calls run in Horizon workers.
- **Endpoints in use:** `POST /brand/retrieve` with `type: by_transaction`, `high_confidence_only: true` (10 credits) — https://docs.context.dev/api-reference/brand-intelligence/brand, guide https://docs.context.dev/guides/enrich-transaction-codes. Sent via `Client::request()` because the generated `brand->retrieve()` helper cannot express a transaction lookup. Any 404, a 400 whose `error_code` is `NOT_FOUND` (the live API's answer for an unidentifiable transaction, 0 credits), or a brand with neither title nor domain → `null` (unresolved: a normal state, not an error); other 400s are rethrown; a domain-only brand uses its domain as the title. The SDK JSON-decodes every error body, so `withApiKey()` also re-wraps non-JSON 4xx/5xx bodies (gateway HTML) as JSON to keep the typed status exception (404 → `NotFoundException`, 5xx → `InternalServerException`); keep that middleware too. A 2xx body that is not a JSON object → `App\Exceptions\ContextDev\ContextDevResponseException`, which extends the SDK's `ContextDevException`, so one catch covers every failure. Only pass hints (`country_gl`/`city`/`mcc`) the feed actually supplied.
- **Operator tool:** `ddev artisan app:resolve-merchant-brand "<descriptor>" [--country=au] [--city=] [--mcc=]` — one billable call per run; do not loop it over the transactions table. For bulk enrichment cache matches, keyed on the descriptor plus every hint sent, per the guide (the Batch API only scrapes/crawls; it cannot resolve transactions).
- **Transaction enrichment (#465):** brands live in the per-user `merchant_brands` sidecar keyed by `(user_id, merchant_key)` (`App\Models\MerchantBrand`, status `resolved`|`unresolved`|`vetoed`; `partial` is a flag on a resolved row). **Never write `transactions.merchant_name`**: it feeds `merchant_key`, which drives rules, the rule backlog and recurring clusters. `App\Services\MerchantBrands\DescriptorGate` decides what may leave the app and **sends only the redacted `MerchantSignature` form** (raw descriptors carry card masks, auth codes, policy/member numbers); it denies credits, transfer pairs, folded fees, transfer/BPAY/PayID/Osko/ATM/fee/income wording, rail-only descriptors (`VISA -PAYPAL *PYPL PAYIN4 AU`, `Afterpay afterpay.com AU`) PayPal payees shaped like a person's name (`PAYPAL *JOHN SMITH`, even behind a card marker) and, on descriptors with no leading card-scheme marker (VISA/MASTERCARD/MC/EFTPOS) or non-PayPal rail, short letters-only shapes with no business cue word or any honorific-led name (`JOHN SMITH`, `PAY JOHN SMITH`, `MR JOHN ANDREW SMITH`; generic PAY/DEBIT/CARD prefixes are stripped, never trusted; plainly named merchants like `JB HI-FI HOBART` are the accepted cost). Hints sent: only what the row carries (Basiq suburb, Redbark MCC); **no invented country** (`country_gl` constrains the match and many merchants are foreign). `EnrichMerchantBrandsJob` (dispatched after CSV/Basiq/Redbark imports) queues `ResolveMerchantBrandJob` per key with ≥2 debit rows and no vetoed/fresh row, most frequent first, sized to the remaining budget; both run on the `enrichment` queue, drained by its own single-process `supervisor-enrichment` in `config/horizon.php` (queue order on an auto-balanced supervisor does not set priority). Retry windows: resolved 180d, partial 30d, unresolved 14d; vetoed never. Spend controls: `CONTEXT_DEV_ENRICHMENT_ENABLED` (default **false**) and `CONTEXT_DEV_DAILY_CREDIT_CAP` (default 500 credits per Australia/Brisbane day, `ContextDevCreditBudget`). UI: `TransactionList` shows logo + brand name beside the original descriptor, "Identify merchant" (user-requested lookup, still gated) and "Wrong merchant" (veto).
- **Enrichment runtime (#470):** the #465 switches resolve through `services.context_dev.enrichment_enabled` and `services.context_dev.daily_credit_cap`. `.env.example` ships them off / 500; staging runs enabled with a cap of 200. Locally the `enrichment` queue only drains while Horizon runs: `op horizon`, or `op dev`, which already starts it (`composer run dev`). Don't run both.
...
- **Pest 4** (`pestphp/pest` v4.4.6) over PHPUnit 12. Suites in `phpunit.xml`: `Unit`, `Feature`, `Browser`, `Arch` (`tests/Arch.php`).
