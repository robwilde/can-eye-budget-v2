...
- **Enrichment runtime (#470):** enabled by `CONTEXT_DEV_ENRICHMENT_ENABLED=true` (`services.context_dev.enrichment_enabled`, default `false`) and capped by `CONTEXT_DEV_DAILY_CREDIT_CAP` (`services.context_dev.daily_credit_cap`, default 500; local and staging use 200). Jobs run on the `enrichment` queue (`supervisor-enrichment`, 1 process, in `config/horizon.php` defaults), so locally they only drain while `op horizon` is running.
