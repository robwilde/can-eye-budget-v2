...
Always use `op make.migration` (or `ddev exec php artisan make:migration`) for consistent naming and timestamps.
...
```bash
op make.migration create_posts_table
op make.migration add_slug_to_posts_table
```
