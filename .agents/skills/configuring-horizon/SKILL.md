...
```bash
ddev exec php artisan horizon:install
```
...
1. Run `op horizon` and visit `/horizon`
...
- The metrics dashboard stays blank until `horizon:snapshot` is scheduled. Running `op horizon` alone does not populate metrics.
