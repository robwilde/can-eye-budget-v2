...
Three additional rules for projects that call LLM APIs, use agent/workflow frameworks (LangGraph, CrewAI, custom state machines), or wire up observability/telemetry (Langfuse, LangSmith, OpenTelemetry). Apply these on top of the ten core rules.

## Rule 11: Prompt tests — test the contract, not the content
...
## Rule 12: Observability is infrastructure
...
## Rule 13: Agent and flow tests test transitions
...
- **Must fix:** Rule 13 violations that assert prompt strings or call counts — they break on every model/prompt change
- **Should fix:** Rule 11 wording assertions
- **Worth noting:** Rule 12 — flag it, but don't block a small change on it
